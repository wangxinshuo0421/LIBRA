...
uint32_t CustomHeader::GetSerializedSize (void) const{
    ...
    if (headerType & L4_Header){
        if (l3Prot == 0x6) // TCP
            ...
            len += GetUdpHeaderSize(headerType & SEANET_Header);
  ...
  Buffer::Iterator i = start;
    ...
    if (headerType & L4_Header){
        if (l3Prot == 0x6){ // TCP
            ...
            if (headerType & SEANET_Header){
                // SeanetHeader
                i.WriteHtonU32 (udp.nextaddr);
                i.WriteHtonU32 (udp.dstaddr);
            }
            udp.ih.Serialize(i);
        }else if (l3Prot == 0xFF){ // CNP
            i.WriteHtonU16(cnp.fid);
            i.WriteU8(cnp.qIndex);
            i.WriteU8(cnp.ecnBits);
            i.WriteHtonU32(cnp.seq);
            i.WriteHtonU32(cnp.nextaddr);
            i.WriteHtonU32(cnp.dstaddr);
...
CustomHeader::Deserialize (Buffer::Iterator start)
{
    ...
    if (headerType & L4_Header){
        if (l3Prot == 0x6){ // TCP
            ...
            l4Size = tcp.length * 4;
        }else if (l3Prot == 0x11){ // UDP + SeqTsHeader + SeanetHeader
            i = start;
            i.Next(l2Size + l3Size);
            // udp header
            ...
            if (headerType & SEANET_Header){
                // SeanetHeader
                udp.nextaddr = i.ReadNtohU32();
                udp.dstaddr = i.ReadNtohU32();
            }
            l4Size = GetUdpHeaderSize(headerType & SEANET_Header);
        }else if (l3Prot == 0xFF){
            cnp.fid = i.ReadNtohU16();
            cnp.qIndex = i.ReadU8();
            cnp.ecnBits = i.ReadU8();
            cnp.seq = i.ReadNtohU32();
            cnp.nextaddr = i.ReadNtohU32();
            cnp.dstaddr = i.ReadNtohU32();
            l4Size = 16;
        }else if (l3Prot == 0xFC || l3Prot == 0xFD){ // ACK or NACK
            ack.sport = i.ReadU16();
            ack.dport = i.ReadU16();
...
uint32_t CustomHeader::GetUdpHeaderSize(bool seanet_enable){
    if (seanet_enable)
	    return 8 + sizeof(udp.pg) + sizeof(udp.seq) + sizeof(udp.nextaddr) + sizeof(udp.dstaddr) + IntHeader::GetStaticSize();
    else
        return 8 + sizeof(udp.pg) + sizeof(udp.seq) + IntHeader::GetStaticSize();
}

uint32_t CustomHeader::GetStaticWholeHeaderSize(bool seanet_enable){
	return 14 + 20 + GetUdpHeaderSize(seanet_enable);
}
