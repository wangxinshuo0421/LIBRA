...
RdmaClient::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::RdmaClient")
    ...
    .AddAttribute ("DestPort",
                   ...
                   MakeUintegerChecker<uint16_t> ())
    .AddAttribute ("DestId",
                   "Dest device id in topo file",
                   UintegerValue (0),
                   MakeUintegerAccessor (&RdmaClient::m_dst_id),
                   MakeUintegerChecker<uint16_t> ())
...
void RdmaClient::SetRemote (Ipv4Address ip, uint16_t port, uint16_t id)
{
  m_dip = ip;
  m_dport = port;
  m_dst_id = id;
...
void RdmaClient::StartApplication (void)
{
  ...
  rdma->AddQueuePair(m_size, m_pg, m_sip, m_dip, m_sport, m_dport, m_dst_id, m_win, m_baseRtt, MakeCallback(&RdmaClient::Finish, this));
}
