...
RdmaClientHelper::RdmaClientHelper (uint16_t pg, Ipv4Address sip, Ipv4Address dip, uint16_t sport, uint16_t dport, uint16_t dst_id, uint64_t size, uint32_t win, uint64_t baseRtt)
{
	...
	SetAttribute ("DestId", UintegerValue (dst_id));
