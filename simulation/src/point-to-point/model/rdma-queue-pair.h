...
struct SeqMgr{
    ...
    Ipv4Address src_ip;
    SeqMgr() : acked(false), seq(0), size(0), expect_ack(0), last_send_time(0), next_send_time(0) {}
    SeqMgr(uint32_t _seq, uint32_t _size, uint64_t _last_send_time, uint64_t _next_send_time, Ipv4Address _src_ip) {
        acked = false;
        seq = _seq;
        size = _size;
        expect_ack = seq + size;
        last_send_time = _last_send_time;
        next_send_time = _next_send_time;
        src_ip = _src_ip;
    }
...
public:
    ...
    std::vector<SeqMgr> m_outstanding_pkts; // seqmap qp windows  map<qp, SeqMgr>
	...
	RdmaQueuePair(uint16_t pg, Ipv4Address _sip, Ipv4Address _dip, uint16_t _sport, uint16_t _dport, uint16_t _dst_id);
