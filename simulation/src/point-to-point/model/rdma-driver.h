...
public:
    ...
    // add a queue pair
    void AddQueuePair(uint64_t size, uint16_t pg, Ipv4Address _sip, Ipv4Address _dip, uint16_t _sport, uint16_t _dport, uint16_t _dstId, uint32_t win, uint64_t baseRtt, Callback<void> notifyAppFinish);
