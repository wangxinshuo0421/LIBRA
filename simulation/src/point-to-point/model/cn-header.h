...
//Getters
  ...
  void SetSEANextAddr (const uint32_t nextaddr);
  uint32_t GetSEANextAddr () const;

  void SetSEADstAddr (const uint32_t dstaddr);
  uint32_t GetSEADstAddr () const;
...
private:
  ...
  // SeanetHeader
  uint32_t m_nextaddr;
  uint32_t m_nextaddr_ntoh;

  uint32_t m_dstaddr;
  uint32_t m_dstaddr_ntoh;
};
