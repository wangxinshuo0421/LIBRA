...
int RdmaEgressQueue::GetNextQindex(bool paused[]){
    ...
    uint64_t next_retrans_time = UINT64_MAX;
    for (qIndex = 1; qIndex <= fcount; qIndex++){
        uint32_t idx = (qIndex + m_rrlast) % fcount;
        Ptr<RdmaQueuePair> qp = m_qpGrp->Get(idx);
        for (auto it = qp->m_outstanding_pkts.begin(); it != qp->m_outstanding_pkts.end(); ++it) {
            if ((it->acked==false) && (next_retrans_time>it->next_send_time)){
                next_retrans_time = it->next_send_time;
            }
        }
        if (!paused[qp->m_pg] && ((next_retrans_time <= now_time) || (qp->GetBytesLeft() > 0) && (!qp->IsWinBound()))){
            if (m_qpGrp->Get(idx)->m_nextAvail.GetTimeStep() > now_time) { //not available now
                continue;    
            }
            res = idx;
            break;
...
void QbbNetDevice::SendCnp(Ptr<Packet> cnp, CustomHeader &ch){
    SwitchSend(0, cnp, ch);
}
