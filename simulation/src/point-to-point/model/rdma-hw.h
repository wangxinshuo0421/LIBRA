...
public:
    ...
    std::map<uint32_t, std::vector<Ipv4Address> > next_ip_table; // map from dst node id to leaf ip (uint32_t) 
    std::map<uint32_t, std::vector<uint16_t> > next_ip_weight_table;
    std::map<uint32_t, std::vector<uint64_t> > next_ip_timestamp;
    ...
    void SetNextIpTable(std::map<uint32_t, std::vector<Ipv4Address> > _next_ip_table);
    void Setup(QpCompleteCallback cb); // setup shared data and callbacks with the QbbNetDevice
    void ScheduleRetransWin();
    ...
    void AddQueuePair(uint64_t size, uint16_t pg, Ipv4Address _sip, Ipv4Address _dip, uint16_t _sport, uint16_t _dport, uint16_t dstId, uint32_t win, uint64_t baseRtt, Callback<void> notifyAppFinish); // add a new qp (new send)
    ...
    bool TransferWeight(uint32_t dst_id, uint16_t congestion_path_id);
    ...
    Ipv4Address GetRandNextIp(uint16_t dst_id); // calculate a random next ip
    void GenPacket(Ptr<Packet> p, Ptr<RdmaQueuePair> qp, uint32_t seq); // gen a packet with input
    Ptr<Packet> GetRetransPacket(Ptr<RdmaQueuePair> qp); // get next packet to send in retransmission sets
