...
TypeId SwitchNode::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::SwitchNode")
	...
	.AddAttribute("isLeafSwitch",
			"mark Leaf Switch.",
			BooleanValue(false),
			MakeBooleanAccessor(&SwitchNode::m_is_leaf_sw),
			MakeBooleanChecker())
...
void SwitchNode::MatchLocal(Ptr<Packet> p, CustomHeader &ch){
    auto local_entry = local_ip_table.find(ch.dip);
    if (local_entry == local_ip_table.end()) {
        // std::cout<< "entry null" << std::endl;
        // p->Print(std::cout);
    } else {        // match local ip ; exchange pkt's extend ip and dst ip
        PppHeader ppp;
        Ipv4Header ipv4;
        UdpHeader udp;    
        SeqTsHeader seqTs;
        ...
        p->RemoveHeader(seanet);
        ipv4.SetDestination(Ipv4Address(seanet.GetSEADstAddr()));
        ch.dip = seanet.GetSEADstAddr();
        p->AddHeader(seanet);
        p->AddHeader(seqTs);
        p->AddHeader(udp);
        p->AddHeader(ipv4);
...
Ptr<Packet> SwitchNode::GenFastCnp(Ptr<Packet> ori_pkt, CustomHeader & ch){
    // add SeanetHeader
    CnHeader cn_hdr;
    cn_hdr.SetSeq (ch.udp.seq);
    cn_hdr.SetQindex (ch.udp.pg);
    cn_hdr.SetFlow (ch.udp.sport);
    cn_hdr.SetSEANextAddr (ch.udp.nextaddr);
    cn_hdr.SetSEADstAddr (ch.udp.dstaddr);

    Ptr<Packet> cnp = Create<Packet>(std::max(64-14-20-16, 0));
    cnp->AddHeader(cn_hdr);

    // add ip header
    Ipv4Header ipv4;	// Prepare IPv4 header
    ipv4.SetDestination(Ipv4Address(ch.sip));
    ipv4.SetSource(Ipv4Address(ch.dip));
    ipv4.SetProtocol(0xFF); //ack=0xFC nack=0xFD cnp=0xFF
    ipv4.SetTtl(64);
    ipv4.SetPayloadSize(cnp->GetSize());
    cnp->AddHeader(ipv4);

    PppHeader ppp;
	ppp.SetProtocol(0x0021);    //eth_ipv4: 0x0800 -> 0x0021; eth_ipv6: 0x86DD -> 0x0057;
